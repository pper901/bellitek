...
//     ? new URL(document.referrer).pathname.trim()
//     : null;
//   console.log(referrerPath);
//   if (referrerPath.startsWith('/lecturer/classes/')) {
//     console.log('✅ Came from class show page');
// } else {
//     console.log('❌ Did NOT come from class show page');
// }
...
// }else if (referrerPath.startsWith('/lecturer/classes/')){
...
//   console.log("data value is ",data);
      ...
      */
      ...
      const className = document.getElementById("classTitle").value;
      const classUuid = document.getElementById("classUuid").value.trim();
      console.log(className+","+nm);
      ...
      socket.onopen = (event) => {
          ...
          let jsonObject = { JOIN: true, classUuid: classUuid, id: classUuid, name: nm, className: className, title: "Lecturer" };
              ...
              switch(key){
                ...
                case "newUser":
                  ...
                  while(result.length > 0){
                    ...
                    var newDiv = document.createElement("div");
                      ...
                      newDiv.onclick = handleUserPeer;
                      sidePanel.appendChild(newDiv);
                      userBase.push({id: resi1[1], name: resi1[0]});
                ...
                case "ready":
                  if(!startButton.disabled){
                    ...
                    console.log("Startbutton is disabled");
  ...
  if(modal && modal.querySelector(".tab-menu")){
    ...
    closeTab.addEventListener("click", function() {
      ...
      if (allClTb.length > 1) {
        ...
        socket.send(JSON.stringify({id: id, classUuid: classUuid, closeTab: closeIndex}));
    });
    ...
    tabText.addEventListener('click', function() {
      ...
      socket.send(JSON.stringify({id: id, classUuid: classUuid, changeToTab: activeIndex}));
    ...
    //send to others
    socket.send(JSON.stringify({id: id, classUuid: classUuid, createNewTabMenu: "newTab"}));
  }
}
...
function showAllFileResources(files) {
  ...
  function handleBackVideo(){
      ...
      socket.send(JSON.stringify({id: id, classUuid: classUuid, backTheVideo: "lecturer"}));
  }
    ...
    // document.getElementById("u-btn").addEventListener("click", sendFile);
      ...
      async function setMessage() {
        ...
        if (fmessage.trim() !== "") {
            ...
            let jOb = { id: id, classUuid: classUuid, chatMessage: nm + "&^ " + message };
    ...
    async function showEditResourceFrame() {
      ...
      socket.send(JSON.stringify({id: id, classUuid: classUuid, editResourceFrame: "edit"}));
      document.getElementById("userEditPerm").addEventListener("change", function(){
        ...
        socket.send(JSON.stringify({id: id, classUuid: classUuid, userEditPerm: document.getElementById("userEditPerm").value }));
      });
      document.getElementById('filename').addEventListener("input", function(){
        ...
        if(filen.trim() !== null && filen.trim() !== ""){
          if(modal && modal.querySelector(".tab-menu")){
            ...
            socket.send(JSON.stringify({id: id, classUuid: classUuid, activeTabName: filen.trim()}));
          }
        }
      });
  }
  ...
  function runEditorSettings(){
    ...
    editor.addEventListener('input', function() {
        ...
        //for the tabs save the content in a queue
        updateEditorQueue();
        socket.send(JSON.stringify({id: id, classUuid: classUuid, editorText: editor.innerHTML}));
        // wrapTags('tag');
    });
  ...
  function saveTheFile() {
      ...
      function showFrame() {
        ...
        if (link.trim() !== "") {
            ...
            socket.send(JSON.stringify({id: id, classUuid: classUuid, visitLink: link}));
           ...
           // Assuming addr is your iframe element
            addr.addEventListener('load', () => {
              ...
              iframeDocument.addEventListener('scroll', () => {
                  ...
                  socket.send(JSON.stringify({ id: id, classUuid: classUuid, scrolling: `${scrollTop};${scrollLeft}` }));
              });
            });
    ...
    function closeModalP(){
      ...
      socket.send(JSON.stringify({id: id, classUuid: classUuid, closeLinkFrame: "closeFrame"}));
    }
      ...
      function extractValuesByKey(jsonString, key) {
          try {
              ...
              // Fix broken JSON by extracting only valid JSON object
              ...
              remoteVideo.oncanplay = () => {
                ...
                player.onclick = async () => { 
                  try {
                    ...
                    socket.send(JSON.stringify({id:id, classUuid: classUuid, streamPerson: jsonObject.name}));
      ...
      function sendKeepAlive() {
        ...
        if (socket.readyState === WebSocket.OPEN) {
            ...
            socket.send(JSON.stringify({classUuid: classUuid, keepalive: 'keep-alive'}));
        }
      }
