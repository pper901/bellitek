...
const referrerPath = document.referrer
  ...
  if (referrerPath.startsWith('/classroom/student')) {
...
}else if (referrerPath.startsWith('/classroom/student')){
const data = sessionStorage.getItem("lecScriptLoaded");
console.log("This is student class.html");
  ...
  */
  ...
  if(prot == "https:"){
    ...
    socket.onopen = (event) => {
        ...
        let jsonObject = {JOIN: true, classUuid: classUuid, id: userId, name: nm, className: className, title: role};
...
function endTheCall(){
window.location.href =  `${window.location.protocol}//${window.location.host}/classroom/student`;
}
  ...
  async function handleOffer(offer){
    ...
    pc.ontrack = (e) => {
      ...
      if(newPeerJson.title === "lecturer"){
        console.log("Received from lecturer");
