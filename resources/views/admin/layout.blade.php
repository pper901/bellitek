...
<!-- TEMPORARILY DISABLED: @vite(['resources/css/app.css', 'resources/js/app.js']) -->
<!-- ADDED SIMPLE STYLING TO ENSURE PAGE VISIBILITY -->
...
<a href="{{ route('admin.guides.index') }}" class="block px-3 py-2 rounded hover:bg-gray-800 {{ request()->routeIs('admin.guides.index') ? 'bg-gray-800' : '' }}">📚 Guides</a>
...
<main class="flex-1 p-6 overflow-y-auto">
    <header class="flex justify-between items-center mb-6">
        ...
        <!-- TEMPORARILY WRAPPING AUTH CALL IN A CHECK TO PREVENT CRASH -->
        <div class="text-gray-700">Welcome, @if(auth()->check()){{ auth()->user()->name }}@else Admin @endif</div>
    </header>
