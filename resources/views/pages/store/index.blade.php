...
<div class="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 bg-gray-50">

<!-- Hero Section / Banner -->
<header class="bg-blue-800 rounded-xl shadow-2xl p-8 md:p-16 mb-12 text-white">
    <h1 class="text-4xl md:text-6xl font-extrabold mb-3 leading-tight">
        Repair Revolution Starts Here
    </h1>
    <p class="text-xl md:text-2xl font-light mb-8 opacity-90">
        Find the tools, parts, and devices you need to fix anything.
    </p>
    ...
    <form action="{{ route('store.search') }}" method="GET" class="max-w-2xl">
        <div class="relative">
            <input 
                ...
                class="w-full text-gray-800 border-none p-4 pr-16 rounded-lg shadow-inner focus:ring-4 focus:ring-blue-300 transition"
                placeholder="Search for Pro Tech Kits, iPhone screens, or specific parts..."
            >
            <button type="submit" class="absolute right-0 top-0 h-full px-4 text-blue-600 hover:text-blue-800 transition">
...
@php
    // Define the categories and their associated data variables
    $categories = [
        'Tools' => ['type' => 'tool', 'products' => $tools ?? []],
        'Device Parts' => ['type' => 'part', 'products' => $parts ?? []],
        'Devices' => ['type' => 'device', 'products' => $devices ?? []],
    ];
@endphp
...
@foreach($categories as $title => $data)
    <section class="mb-14">
        <div class="flex justify-between items-center mb-6 border-b border-gray-200 pb-2">
            ...
            <a href="{{ route('store.category', $data['type']) }}" 
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
            @forelse($data['products'] as $product)
                ...
                <div class="bg-white border border-gray-100 rounded-xl shadow-lg hover:shadow-xl 
                    ...
                    <div class="p-4">
                        ...
                        <!-- Rating (Hardcoded for now, ideal for dynamic) -->
            ...
            @empty
                <p class="text-gray-500 italic col-span-full text-center py-6 border-dashed border-2 p-8 rounded-lg">
                    We are currently out of stock for {{ $title }}. Check back soon!
                </p>
            @endforelse
        </div>
    </section>
