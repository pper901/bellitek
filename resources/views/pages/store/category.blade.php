...
<div class="mb-8 border-b border-gray-200 pb-4">
    <nav class="text-sm text-gray-500 mb-2">
        <a href="{{ route('store.index') }}" class="hover:text-blue-600">Home</a>
    ...
    <p class="text-gray-600 mt-1">
        Explore our curated selection of high-quality {{ str_replace('_', ' ', $category) }} products.
    </p>
</div>

<!-- Filtering and Sorting Bar -->
<div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 bg-white p-4 rounded-lg shadow-sm border border-gray-100">
    
    <!-- Filter/Search Placeholder (Requires Controller Logic) -->
    <form action="{{ url()->current() }}" method="GET" class="w-full md:w-auto flex flex-col sm:flex-row gap-4">
        
        <!-- Sort By -->
        <select name="sort" onchange="this.form.submit()"
                class="border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
            <option value="latest" @selected(request('sort') == 'latest' || !request('sort'))>Sort by Latest</option>
        ...
        <select name="condition" onchange="this.form.submit()"
            ...
            <option value="fairly used" @selected(request('condition') == 'fairly used')>Fairly Used</option>
        </select>

        <!-- Optional Search within Category -->
        <div class="relative w-full sm:w-64">
            <input type="text" name="search" placeholder="Search this category..."
                   value="{{ request('search') }}"
                   class="w-full border-gray-300 rounded-lg shadow-sm pr-10 focus:border-blue-500 focus:ring-blue-500 text-sm">
    ...
    <p class="text-sm text-gray-500 mt-3 md:mt-0">
...
<div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
    @forelse($products as $product)
        <div class="bg-white border border-gray-100 rounded-xl shadow-lg hover:shadow-xl 
                    transform hover:-translate-y-1 transition-all duration-300 relative overflow-hidden">
            ...
            <div class="p-4">
                ...
                <a href="{{ route('store.product', $product->id) }}" 
                   class="text-lg font-bold text-gray-900 hover:text-blue-600 transition line-clamp-2"
                   title="{{ $product->name }}">
                    {{ $product->name }}
                </a>
                ...
                <p class="text-sm text-gray-500 mt-1">
                    {{ $product->brand ?? 'Generic' }} | {{ ucfirst($product->condition) }}
                </p>

                <!-- Price -->
                <p class="text-xl text-green-700 font-extrabold mt-2">
    ...
    @empty
        <div class="col-span-full text-center bg-white p-12 rounded-xl shadow-inner border-2 border-dashed border-gray-300">
            <p class="text-2xl font-semibold text-gray-700 mb-2">Nothing found in this category.</p>
            <p class="text-gray-500">Try adjusting your filters or search terms.</p>
            <a href="{{ route('store.index') }}" class="mt-4 inline-block text-blue-600 hover:text-blue-700 font-medium">&larr; Back to all products</a>
        </div>
    @endforelse
</div>
